...
public class AuthorizationServerAuthConfig extends WebSecurityConfigurerAdapter {
	...
	public void configure(WebSecurity web) throws Exception {
		web.ignoring().antMatchers("/public/**", "/error**", "/health/**", "/adminuser**", "/actuator**",
				"/favicon.ico", "/usermgmt/**", "/notification-xray**", "/notification-service-info**", "/ns-status**", "/notification-health-status**", "/health-status**", "/users**", "/user**",
				"/swagger-ui/**",
				"/swagger-resources/**",
				"/swagger-ui.html",
				"/v2/api-docs",
				"/webjars/**",
				"/user/**", "/status/**", "/h2-console/**", "/hello**", "/app1/**", "/app2/**");
	}
